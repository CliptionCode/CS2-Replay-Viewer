...
> **Application version:** `0.1.6`. This version adds a PayPal donation button to the Welcome screen and restores reliable mouse-wheel zooming and mouse-drag panning after player deselection by keeping replay viewport interactions on the replay container with an explicitly non-passive wheel listener. Keep `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, and the application package entry in `src-tauri/Cargo.lock` on the same version when releasing later changes.
