...
> **Application version:** `0.1.5`. This version adds authoritative smoke-extinguished inferno lifetimes, complete conditional kill-feed indicators and flash-assister names, retained camera position on player deselection, seamless in-play player selection, and non-selectable score/bomb-status labels. Keep `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, and the application package entry in `src-tauri/Cargo.lock` on the same version when releasing later changes.
